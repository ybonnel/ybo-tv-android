...
import fr.ybo.ybotv.android.service.AllocineService;
...
public class AllocineRatingLoader {
    ...
    public AllocineRatingLoader(Context context) {
    ...
    private void queueRating(Programme programme, ImageView imageView) {
        ...
        executorService.submit(new RatingLoader(p));
    }
    ...
    private Integer getResource(Programme programme) {
        //from web
        try {
            Float result = AllocineService.getInstance().getMovieRating(programme);
            if (result != null) {
    ...
    class RatingLoader implements Runnable {
        RatingToLoad ratingToLoad;

        RatingLoader(RatingToLoad ratingToLoad) {
